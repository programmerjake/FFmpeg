...
static int time_tracker_set_time(TimeTracker* tt, const MCCTimecode* tc, void* log_ctx)
{
    ...
    int ret =
        av_timecode_init_from_components(&tt->timecode, last.rate, last.flags, tc->hh, tc->mm, tc->ss, tc->ff, log_ctx);
    if (ret < 0) {
        tt->timecode = last;
        return ret;
    }
...
static int mcc_read_header(AVFormatContext* s)
{
    ...
    while (!ff_text_eof(&tr)) {
        ...
        coded_anc.wrapping_type =
            tc.field ? SMPTE_436M_WRAPPING_TYPE_VANC_FIELD_2 : SMPTE_436M_WRAPPING_TYPE_VANC_FRAME;
        ...
        if (mcc->eia608_extract) {
            Smpte291mAnc anc;
            if (avpriv_smpte_291m_anc_decode(
            ...
            ret = avpriv_smpte_436m_anc_append(sub, 1, &coded_anc);
            if (ret < 0)
                return ret;
        }
