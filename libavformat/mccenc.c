...
static int mcc_write_packet(AVFormatContext* avf, AVPacket* pkt)
{
    ...
    Smpte436mAncIterator iter;
    ret = avpriv_smpte_436m_anc_iter_init(&iter, pkt->data, pkt->size);
    if (ret < 0)
        return ret;
    Smpte436mCodedAnc coded_anc;
    while ((ret = avpriv_smpte_436m_anc_iter_next(&iter, &coded_anc)) >= 0) {
        Smpte291mAnc anc;
        ret = avpriv_smpte_291m_anc_decode(
